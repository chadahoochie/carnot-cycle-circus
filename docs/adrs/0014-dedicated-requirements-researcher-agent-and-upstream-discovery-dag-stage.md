# ADR-0014: Dedicated Requirements Researcher Agent and Upstream Discovery DAG Stage 🔬📋
...
- **Cognitive Coupling (Exploratory vs. Structured)**: Research is an exploratory, convergent cognitive task; writing rigorous acceptance criteria and decomposing ticket DAGs is a structured, generative task.
...
## Decision
We introduce a dedicated **Requirements Researcher Agent** (`AgentRole.RequirementsResearcher`) positioned as **Stage 1 (Upstream Discovery & Feasibility)** in the Directed Acyclic Graph (DAG) before the Technical Product Manager:
...
2. **Upstream Discovery DAG Pipeline Stage (`WorkflowGraph` & `GraphWorkflowExecutor`)**:
...
- **Dynamic Subagent Delegation by TPM**: Rejected in favor of an explicit visual DAG stage (Option A) to ensure first-class visual observability, independent step execution, and clear telemetry logging on the visual canvas.
...
- ✅ 100% test coverage across all role execution, DAG routing, and artifact categorization suites.
