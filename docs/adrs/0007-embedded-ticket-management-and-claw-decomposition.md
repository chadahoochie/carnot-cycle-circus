# ADR-0007: Embedded Ticket Management & CLAW Work Decomposition
...
2. **Automated TPM/Architect Decomposition**:
   ...
   - `DeconstructStoryIntoTechnicalSubtasks`: Lead Architect generates 5 atomic subtasks with explicit CLAW dependencies (`DependsOnTicketIds`).
...
4. **State Machine & CLAW Scheduling**: `HandoffRouter` evaluates dependency satisfaction and transitions downstream backlog tickets to `Ready` when prerequisite tickets complete.
...
### Positive
- ✅ Structured work tracking with visual Kanban board, backlog view, and dependency CLAW visualization.
...
### Negative / Trade-offs
- ⚠️ Adds domain modeling complexity around ticket state transitions and CLAW resolution.
